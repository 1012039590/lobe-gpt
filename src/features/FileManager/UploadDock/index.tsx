...
const UploadDock = memo(() => {
  ...
  const fileList = useFileStore(fileManagerSelectors.dockFileList);
  const totalUploadingProgress = useFileStore(fileManagerSelectors.overviewUploadingProgress);
  const overviewUploadingStatus = useFileStore(fileManagerSelectors.overviewUploadingStatus);
  ...
  useEffect(() => {
    if (show) return;
    if (overviewUploadingStatus === 'uploading') setShow(true);
  }, [overviewUploadingStatus, show]);
