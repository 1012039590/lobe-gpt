  ...
  async query({
    ...
    const chunksList = await serverDB
      .select({
        ...
        filename: files.name,
        id: chunks.id,
        messageId: messageFileChunks.messageId,
        text: chunks.text,
      })
      .from(messageFileChunks)
      .leftJoin(chunks, eq(chunks.id, messageFileChunks.chunkId))
      .leftJoin(files, eq(chunks.fileId, files.id))
      .where(inArray(messageFileChunks.messageId, messageIds));
  ...
  async create(
    ...
    return serverDB.transaction(async (trx) => {
      ...
      if (fileChunks && fileChunks.length > 0 && ragQueryId) {
        await trx.insert(messageFileChunks).values(
